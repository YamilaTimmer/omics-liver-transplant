...
```{r}
load("/students/2024-2025/Thema08/liver-transplant/proteomics/data_norm_imputed_newest_30min.Rdata")
```
...
df_data <- as.data.frame(assay(data_imp_30min))
...
```{r}
model <- pca(t(assay(data_imp_30min)), center = T, scale = T)
```
...
ggsave("../../plots/pca_30_min.svg", last_plot(), bg='transparent')
...
```{r}
#| eval: false
save(top_x_genes, file = "/students/2024-2025/Thema08/liver-transplant/proteomics/top_x_genes_30_new_new.Rdata")
```
...
```{r}
load("/students/2024-2025/Thema08/liver-transplant/proteomics/data_norm_imputed_newest_150min.Rdata")
```
...
ggplot(
  ...
  aes(x = `Comp 1`,
      ...
      shape = meta_data_liver_filtered_150$Transplant),) +
  geom_point() +
  theme_minimal() +
  labs(color = "Billiary viability score",
       shape = "Transplanted")
...
```{r}
#| eval: false
ggsave("../../plots/pca_150_min.svg", last_plot(), bg='transparent')
```
...
```{r}
#| eval: false
save(top_x_genes_150, file = "/students/2024-2025/Thema08/liver-transplant/proteomics/top_x_genes_150_new_new.Rdata")
```
