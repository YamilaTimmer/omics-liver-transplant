...
```
**Conclusion**: most of the proteins are identified in all of the samples
...
```
**Conclusion**:
On average the missing values come from proteins with low intensities, this points to "missing not at random" (MNAR) values, imputation methods that are suited for this are: 
...
```
**Conclusion**:
- The effects of imputing are visible, after imputing, the missing values are filled with a "peak" on the left side of the x-axis, with replacement values
