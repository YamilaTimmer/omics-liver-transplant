...
**Conclusion**: The missingness is higher for the 150 minute samples than for the 30 minute samples. 
...
Below the new missingness percentages are calculated, it seems removing the 'bad' samples did not have a big impact on the overall missingness across samples. The missingness in both 30 min and 150 min only went down by 1%.
```{r}
...
Filtering proteins with high missingness, here all proteins that have more than 4 missing values in either of the conditions (high/low biliary viability) are removed.
...
```
**Conclusion**:
- The effects of imputing are visible, after imputing, the missing values are filled with one or more "peaks" on the left side of the x-axis, containing the lowest-found values in that sample which have been used for imputation.
...
head(low_vs_high_150min_entrez)
```
...
- One significant pathway was found, `hsa00500`, which is a pathway that involves starch and sucrose metabolism. 
...
print(as.data.frame(head(low_vs_high_150min_FC)))
```
**Conclusion:**
...
- The code above generates a pathview visualisation for each significant pathway (based on p-adj <= 0.05) that contains one or more significant DEP's (also based on based on p-adj <= 0.05). In this case that is only hsa00500, as all other pathways in this research are not significant.
