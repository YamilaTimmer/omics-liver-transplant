...
experimental_design_150min <- data.frame(
    label = colnames(samples_150min_data[,4:46]), # has to match sample names in experimental design
    condition = samples_150min_metadata$`Biliary viability score group`,
    replicate = colnames(samples_150min_data[,4:46]) # there are no replicates in this case, but they still need a unique identifier because duplicate rownames are not allowed
)
...
mean_protein_presence_150min <- median(rowMeans(!is.na(assay(data_se_150min)))) * 100
...
data_se_30min_filter <- filter_proteins(data_se_30min, type = "fraction", min = 0.61)

data_se_150min_filter <- filter_proteins(data_se_150min, type = "fraction", min = 0.34)
```
...
eerst imputatie of normalisatie?
